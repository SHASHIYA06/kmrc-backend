...
// Features: Extraction (PDF/IMG/DOCX/XLSX/CSV) → Chunk → Embeddings (Gemini) → VertexDB → Ask
// MCP Server for Model Control and Training
...
app.post("/ask", async (req, res) => {
  try {
    ...
    const hasTabular = results.some(r => 
      r.metadata && (r.metadata.type === "row" || (r.metadata.headers && r.metadata.headers.length > 0))
    );
    
    // Build context with citations
    const contextBlocks = results.map((r, idx) =>
      `[[${idx+1}]] File: ${r.metadata.fileName} (pos ${r.metadata.position})
${r.text}`
    ).join("\n---\n");
...
- ${hasTabular ? "Because context is tabular or user likely needs a matrix:" : "If the user explicitly asks for table/matrix:"}
  • Return results as **pure JSON** with this structure:
    {
...
const PORT = process.env.PORT || 3000;
app.listen(PORT, () => {
  console.log(`🚀 RAG server with VertexDB and MCP Server running on http://localhost:${PORT}`);
